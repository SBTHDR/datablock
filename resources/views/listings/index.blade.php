            ...
            <div class="flex justify-center">
                @foreach ($tags as $tag)
                    <a href="{{ route('listing.index', ['tag' => $tag->slug]) }}" 
                        class="inline-block ml-2 tracking-wide text-sm font-medium title-font py-0.5 px-1.5 border rounded border-indigo-500 uppercase 
                ...
                >
                    ...
                    <div class="md:flex-grow mr-8 flex items-center justify-start">
                        @foreach ($listing->tags as $tag)
                            <span class="inline-block ml-2 tracking-wide text-sm font-medium title-font py-0.5 px-1.5 border rounded border-indigo-500 uppercase 
                            {{ $tag->slug === request()->get('tag') ? 'bg-indigo-500 text-white' : 'bg-white text-indigo-500' }}">
                                {{ $tag->name }}
                            </span>
                        @endforeach
